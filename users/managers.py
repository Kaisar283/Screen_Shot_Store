...
class NewUserManagger(BaseUserManager):
    ...
    def _create_user(self, email, user_name, password, **extra_field):
        user = self.model(email=email, user_name=user_name, **extra_field)
        user.is_active = True
        user.set_password(password)
        return user.save()

    def create_superuser(self, email, user_name, password, **extra_field):
        ...
        return self._create_user(email, user_name, password, **extra_field)
